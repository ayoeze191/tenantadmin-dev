  ...
  <div class="bg-neutral py-5 px-10 w-full overflow-y-scroll h-screen pb-40">
    ...
    <div class="rounded-lg w-full relative">
      <table-component :headers="headers" :data="rolesData">
        ...
        <template #column1="{ entity }">{{ entity.permissions.length || "N/A" }}</template>
        <template #column2="{ entity }">{{ formatDate(entity.dateCreate) || "N/A" }}</template>
        <template #column3="{ entity }">{{ entity.createdBy || "N/A" }}</template>
  ...
  <modal-component ref="viewRoleModal" title="View Role" @close="handleCloseModal('view')" :style="{'max-w-128': true}">
    <div class="w-full">
      ...
      <ul class="w-full flex flex-wrap gap-2">
        <li class="bg-neutral rounded px-2 py-1.5" v-for="permission in selected_Request.permissions" :key="`${permission.functionId}${i}`">
          {{ permission.name }}
        </li>
      </ul>
    </div>
      ...
      <section class="flex flex-wrap justify-between w-full">
        <ul class="flex flex-col gap-2 w-full max-w-56">
          <li class="flex gap-2 cursor-pointer" v-for="(permission, i) in selected_Request.permissions" :key="`${permission.functionId}${i}`" @click="unselectPermission(permission)">
            ...
            <p class="text-secondary text-sm font-normal">{{ permission.name }}</p>
          </li>
        </ul>
  ...
  <modal-component ref="deleteRoleModal" :plain="true" @close="handleCloseModal('delete')" :style="{'max-w-128': true}">
    <div class="px-8 pt-8 w-[510px] mx-auto">
      ...
      <div class="flex w-full">
        <button class="btn btn_danger" @click="handleDeleteRole()">Delete Role</button>
        <button class="btn btn_alternate" @click="this.$refs.deleteRoleModal.closeModal()">Cancel</button>
      </div>
    </div>
  </modal-component>
</template>
...
//   handleEditRole(){
        const payload = {
            ...
            permissions: this.selected_Request.permissions.map(item => item.functionId)
        }
...
unselectPermission(permission) {
  this.selected_Request.permissions = this.selected_Request.permissions.filter(
    (p) => p.functionId !== permission.functionId
  );
  // Immediately add back to available permissions
  if (!this.permissionList.some(p => p.functionId === permission.functionId)) {
...
filterPermissions(entity) {
  ...
  this.permissionList = this.allPermissions.filter(perm => 
    !this.selected_Request.permissions.some(p => p.functionId === perm.functionId)
  );
}
  }
};
