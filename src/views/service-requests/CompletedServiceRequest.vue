  ...
  <div class="bg-neutral py-5 px-4 w-full min-h-screen">
    <a-card class="!bg-white !rounded-2xl !border-0 max-w-5xl mx-auto">
      ...
      <div>
        ...
        />
        <template v-else-if="completedRequests.length > 0">
          <div class="grid gap-4">
            <a-card
              v-for="service in completedRequests"
            ...
            >
              <div class="flex items-center justify-between mb-2">
                ...
                <a-select
                  ...
                  @change="(val) => onStatusChange(service, statusLiterals[val])"
                  :suffixIcon="dropdownIcon()"
                  :disabled="updatingStatusLoading && updatingStatusId === service.serviceRequestId"
                >
                  <template #option="{ value, label }">
                    <a-tag :color="statusTagColor(value)" class="capitalize">{{ label }}</a-tag>
                  </template>
        ...
        <a-empty v-else description="No completed service requests found" class="my-10" />
      </div>
    </a-card>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div class="space-y-4">
            <div class="bg-white border border-gray-100 rounded-lg p-4">
              ...
              <div class="space-y-3">
                <div class="flex justify-between items-center">
                  <span class="text-sm text-gray-600 font-medium">Reference:</span>
                ...
                <div class="flex justify-between items-center">
                  ...
                  <span class="text-sm text-[#23234a]">{{ selected_Request.serviceType || "N/A" }}</span>
                </div>
                <div class="flex justify-between items-center">
                  <span class="text-sm text-gray-600 font-medium">Unit Type:</span>
                  <span class="text-sm text-[#23234a]">{{ selected_Request.unitType || "N/A" }}</span>
                </div>
              </div>
            </div>
            ...
            <div class="bg-white border border-gray-100 rounded-lg p-4">
              ...
              <div class="space-y-3">
                <div class="flex justify-between items-center">
                  <span class="text-sm text-gray-600 font-medium">Tenant(s):</span>
                  <span class="text-sm text-[#23234a]">{{ selected_Request.tenant || "N/A" }}</span>
                </div>
                <div class="flex justify-between items-center">
                  <span class="text-sm text-gray-600 font-medium">Professional:</span>
                  <span class="text-sm text-[#23234a]">
                    {{
                      selected_Request.professional && selected_Request.professional.trim()
            ...
            <div class="bg-white border border-gray-100 rounded-lg p-4">
              ...
              <div class="space-y-3">
                <div class="flex justify-between items-center">
                  ...
                  <span class="text-sm text-[#23234a]">{{ formatDate(selected_Request.dateCreated) }}</span>
                </div>
                <div class="flex justify-between items-center">
                  <span class="text-sm text-gray-600 font-medium">Updated:</span>
                  <span class="text-sm text-[#23234a]">{{ formatDate(selected_Request.dateUpdated) }}</span>
                </div>
              </div>
            </div>
...
import { h } from "vue";
  ...
  computed: {
    ...
    completedRequests() {
      return this.store.getByStatus("Completed", this.serviceLiterals);
    },
    completedCount() {
  ...
  created() {
    // Only fetch if we don't have data or if we don't have completed data
    if (!this.store.hasData() || !this.store.hasStatusData("Completed", this.serviceLiterals)) {
        ...
        async onOk() {
          ...
          try {
            ...
            if (response && response.responseCode === '00') {
              ...
              // Remove from UI if status is no longer completed
              ...
              message.error(response?.responseDescription || 'Failed to update status');
