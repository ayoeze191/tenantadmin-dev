<template>
  <div class="bg-neutral py-5 px-10 w-full overflow-y-scroll h-screen pb-40">
    <h1 class="text-[24px] font-semibold text-txt_dark font-sf">
      PENDING REQUEST
    </h1>
    <section class="list_container mt-[20px]">
      <div class="flex w-full justify-between border-b border-br1 pb-3 mb-4">
        ...
        >
          ...
          <span class="text-primary font-semibold text-[18px]"
      ...
      <ul v-if="pending_request.length > 0">
        <li
          ...
          v-for="request in pending_request"
        >
          <section class="flex w-full justify-between mb-4">
            <p class="font-medium leading-6 text-txt_dark">
              {{ request.subject }}
              <!-- <span class="text-secondary ml-1">Apt T-69, Sean Apartments </span>  -->
            </p>
            <status-select :label="'Update Status:'"></status-select>
          </section>
          <a
      ...
      <div v-else class="mx-auto">
        <p class="text-black w-fit mx-auto">
          No pending service request has been made yet
        </p>
      </div>
    </section>
  </div>
    ...
    handleFetchPendingServiceRequest() {
      FetchServiceRequests().then((response) => {
        if (response.responseCode == "00") {
          this.pending_request = response.serviceRequests.filter(
            (req) => req.service.serviceStatus == 1
          );
