  ...
  <div class="bg-neutral py-5 px-10 w-full overflow-y-scroll h-screen pb-40">
    ...
    <section class="list_container mt-10">
        <div class=" flex w-full justify-between border-b border-br1 pb-3 mb-4">
            <p class="font-medium text-base text-secondary mr-0.5">{{selected_tab}} Service Requests  <span class="text-primary font-semibold text-[18px]">{{selected_tab == 'all' ? serviceRequests.length :serviceRequests.filter((serv) => serviceLiterals[serv.serviceStatus] == selected_tab).length}}</span></p>
        ...
        <ul v-if="selected_tab == 'all'? serviceRequests.length > 0:serviceRequests.filter((serv) => serviceLiterals[serv.serviceStatus] == selected_tab).length">
            <li class="w-full rounded-md px-4 py-2 bg-grey mb-4" v-for="service in selected_tab == 'all' ? serviceRequests:serviceRequests.filter((serv) => serviceLiterals[serv.serviceStatus] == selected_tab)">
                <section class="flex w-full justify-between mb-4">
                    <p class="font-medium leading-6 text-txt_dark">{{service.subject}}
                         <!-- <span class="text-secondary ml-1">{{service.properties[0].name || 'nill' }} </span>  -->
                        </p>
                    
              <StatusDropdown 
 :modelValue="getStatusLabel(service)" 
  @update:modelValue="newStatus => onStatusChange(service, newStatus)"
/>
                </section>
                <a class="text-sm leading-6 text-txt_dark cursor-pointer underline" @click="openModal(service)">View full details</a>
            </li>
        ...
        <div v-else class="mx-auto ">
         <p class="text-black w-fit mx-auto">   No service request has been made yet</p>
        </div>
    </section>
  </div>

    <modal-component ref="viewRequestModal" title="B-29 Brina Apartments" @close="onModalClose" :button_label="'Save Changes'" @submit="handleSubmit">
        <ul class="grid grid-cols-2 gap-11 w-full max-w-[691px] mx-auto">
            ...
            <li class="w-full">
                <div class="flex gap-2">
                    ...
                    <p class="text-txt_dark leading-7">{{selected_Request.serviceType}}</p>
                </div>
            </li>
             ...
             <li>
                <div class="flex gap-2">
                    ...
                    <p class="text-txt_dark leading-7">{{urgencyLevel[selected_Request] || urgencyLevel[0]}} - {{ Math.floor((new Date() - new Date(selected_Request.dateCreated)) / (1000 * 60 * 60 * 24)) }} days </p>
                </div>
            </li>
           <li>
    <StatusDropdown 
    ...
    />
            ...
            <li class="w-full">
                <div class="flex gap-2">
                    ...
                    <p class="text-txt_dark leading-7">{{selected_Request.description}}</p>
                </div>
            </li>
            ...
            <li>
                <div class="flex gap-2">
                    ...
                    <div class="rounded-md py-1.5 px-5 bg-bg1 flex gap-2 justify-center" v-if="selected_Request.imageUrls">
        ...
        <div class="mt-6" v-if="selected_Request.serviceStatus == 3">
            <div class="flex gap-2 flex-col">
                <!-- <input type="text" v-model="" placeholder="Enter Message"/> -->
                 <label for="message" class="text-[#404164] text-xl">Send Notification</label>
<textarea
  id="message"
  rows="4"
...
></textarea>
            ...
            <div class="flex gap-[24px] ">
               ...
               <button class="border-[#000130] border-[1px] text-[#404164]  px-4 py-2 rounded-[4px] mt-3">
...
import { FetchServiceRequests, updateServiceRequest } from '@/api/serviceRequest';
...
export default {
    data() {
        return {
            ...
            selected_Request: {},
            serviceRequests: [],
            serviceLiterals : ["Requested", "Pending", "Ongoing", "Completed", "Terminate"],
            urgencyLevel: [ "Low","Medium","High","Emergency"],
             statusToIndex: { // Maps status strings to their numeric values
      "Requested": 0,
      "Pending": 1,
      "Ongoing": 2,
      "Completed": 3,
    ...
    return this.serviceLiterals[service.serviceStatus];
        ...
        onStatusChangeModal(newStatus){
            this.selected_Request = {...this.selected_Request, serviceStatus: this.statusToIndex[newStatus]}
        },
        handleSubmit(){
