  ...
  <div class="px-4 font-inter h-full">
    ...
    >
      <div class="flex gap-2.5">
        <table-header
          :total-item-count="requestLists.length"
...
export default {
  ...
  methods: {
    ...
    async handleFetchLandlords(page = 1) {
      ...
      fetchWaitingLeases(this.store.userProfile.referenceID, query)
        .then((response) => {
          ...
          if (response.serviceRequests) {
            this.requestLists = response.serviceRequests.items.map((req) => {
              return {
                ...
                status: req.status,
                Date: this.formatDate(req.requestDate),
              };
            });
          } else handleError(response);
        })
