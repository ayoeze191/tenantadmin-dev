...
export const FetchLandlords =  async (query) => {
    try {
      const response = await getApi(`Account/GetAllAccount?PageSize=${query.size}&CurrentPage=${query.page}&AccountType=1&SearchTerm=${query.query}`);
      return response.data;
    } catch (error) {
      handleError(error);
    }
...
export const UpdateAdminStatus =  async (payload) => {
    try {
      const response = await putApi(`AdminUser/UpdateUserActiveStatus?AdminUserID=${payload.id}&AdminUserStatus=${payload.status}`);
      return response.data;
    } catch (error) {
      handleError(error);
    }
